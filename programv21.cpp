...
/////////////////PID_CONTROL///////////////////////////
float Reference, c_error = 0, c_integral = 0.0, c_derivative = 0.0, c_lasterror = 0.0, c_PID = 0, KP = 3, KI = 0, KD = 0.5;
int kecMAX, kecMIN;
///////////////////////////////////////////////////////
int status_pulley1;
int status_pulley2;
...
void limit2()
{
    ...
    if(GPIORead(8)==0) 
    {
        ...
        PID_stop();
        if((statuslim==2)){ kecM1a=800;kecM1=750;kecM2=750; kecMAX = 1000; kecMIN = 600;}
        if((statuslim==0)){ kecM1a=800;kecM1=750;kecM2=750; kecMAX = 1000; kecMIN = 600;}
        while(GPIORead(8)==0){} 
    }
    usleep(10*1000);
    }
...
void PID()
{
    ...
    while(1){
        ...
        if(pTali2 >= (batas_tali-50))
        {
            kecM2 = (kecM2 * (100 - pTali2)) / 50;
        
            if(kecM2<=250) kecM2 = 250;
            kecM1 = kecM2;
        }
        else
        {
...
while(1){
    ...
    if((pTali1 >= batas_tali) && (statuslim!= 0))
    {
        pulley1_stop();
        pulley2_stop();
        statuslim = 4;
...
void baca()
{
    while(1){
        ...
        printf("PID:%0.2f:%0.4f:%0.2f status:%d:%d:%d PresSpeed1:%03d PresSpeed2:%03d err:%0.2f kecM1:%0.2f kecM2:%0.2f tali1:%0.2f tali2:%0.2f load1:%d load2:%d\n", 
        KP,KI,c_PID,GPIORead(7), GPIORead(8), statuslim, dxl_present_speed_1, dxl_present_speed_2, c_error, kecM1, kecM2, pTali1, pTali2, dxl_present_load_1, dxl_present_load_2);
        //printf("%d\n", GPIORead(8));
...
void filel()
{
    ...
    while(1)
    {
        ...
        {
            fprintf(pFile,"%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%0.2f\t%d\t%d\n", KP, KI, KD, pTali1, pTali2, c_error, c_PID, kecM1, kecM2, dxl_present_speed_1, dxl_present_speed_2);
        }
        usleep(20*1000);
    }
