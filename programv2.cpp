...
void pulley1_up(int moving_speed)
{    
    int i=0;
    do{
...
void pulley1_down(int moving_speed)
{
    int i=0;
...
void pulley2_up(int moving_speed)
{    
    int i=0;
    do{
...
void pulley2_down(int moving_speed)
{
    int i=0;
    moving_speed = moving_speed + 1024; 
...
void RemoteRead(){
    while(1){
        if(PWMin(11) >= 1900){
            if(status_pulley1 == PULLEY_DOWN || status_pulley1 == PULLEY_UP)
            {
                pulley1_stop();
            }
            else{
        ...
        if(PWMin(10) >= 1900){
            if(status_pulley1 == PULLEY_UP || status_pulley1 == PULLEY_DOWN)
            {
                pulley1_stop();
            }
            else if(status_snaplock == TUTUP){}
            else{ 
                pulley1_up(1000);
            }
            usleep(10*1000);
...
void read_servolidar()
{
    ...
    {    
        ...
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS1, ADDR_MX_PRESENT_POSITION, &dxl_present_position, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        //read present speed//
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS1, ADDR_MX_PRESENT_SPEED, &dxl_present_speed, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        //read present load//
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS1, ADDR_MX_PRESENT_LOAD, &dxl_present_load, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, DXL_ID_ATAS1, ADDR_MX_PRESENT_VOLTAGE, &dxl_present_voltage, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
		...
		//read present position//
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS2, ADDR_MX_PRESENT_POSITION, &dxl_present_position, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        //read present speed//
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS2, ADDR_MX_PRESENT_SPEED, &dxl_present_speed, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        //read present load//
        dxl_comm_result = packetHandler->read2ByteTxRx(portHandler, DXL_ID_ATAS2, ADDR_MX_PRESENT_LOAD, &dxl_present_load, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
        ...
        //read present voltage//
        dxl_comm_result = packetHandler->read1ByteTxRx(portHandler, DXL_ID_ATAS2, ADDR_MX_PRESENT_VOLTAGE, &dxl_present_voltage, &dxl_error);
        if (dxl_comm_result != COMM_SUCCESS)
        {
            printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
        }
...
void baca()
{
    while(1){
        ...
        printf("PresSpeed:%03d PresLoad:%0.2f% PresVol:%0.2f\n", 
        dxl_present_speed, load, voltage);
...
{    
    ...
    dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, 1, ADDR_MX_MOVING_SPEED, 100, &dxl_error);
    if (dxl_comm_result != COMM_SUCCESS)
    {
      printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
    }
