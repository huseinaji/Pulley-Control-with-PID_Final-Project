...
{    
    ...
    while(1)
    {            
        json_object_set_new( root, "KP", json_real( KP ));
        json_object_set_new( root, "KI", json_real( KI ));
        json_object_set_new( root, "KD", json_real( KD ));
        json_object_set_new( root, "load_dxl", json_real( load ));
        json_object_set_new( root, "voltage", json_real( voltage ));
        json_object_set_new( root, "tali1", json_real( pTali1 ));
        json_object_set_new( root, "tali2", json_real( pTali2 ));
        ...
        json_object_set_new( root, "altitude_box", json_real( Lidar_data ));
...
void limswitch()
{
    while(1)
    {
        if((GPIORead(7)==0) && (status_pulley1!=PULLEY_DOWN)) 
        {
            pulley1_stop();
            pulley2_stop();
            snaplockclose();
        ...
        if((pTali1>=Lidar_data) || (c_error>=20))
        {  
            if(status_pulley1 == PULLEY_UP){
...
void kontrol()
{
    while(1){
        ...
        {            
            ...
            //DecPoint = 500;
            if((GPIORead(7) == 0) && (status_pulley1 == PULLEY_UP)){
                pulley1_stop();
                pulley2_stop();
            }
            else if((DecPoint <= 0) && (status_pulley1 == PULLEY_DOWN))
            {
                DecPoint = 0;
            }
        }
...
void RemoteRead(){
    while(1){
        ...
        if((status_pulley1 == PULLEY_DOWN) && (status_pulley2 == PULLEY_DOWN))
        {
            pulley1_down(kecM1);
            pulley2_down(kecM2);
        }
        
        if((status_pulley1 == PULLEY_UP) && (status_pulley2 == PULLEY_UP))
        {
            pulley1_up(kecM1);
            pulley2_up(kecM2);
        }
...
void baca()
{
    while(1){
        ...
        printf("PID:%0.2f:%0.4f:KecPoin%0.2f status:%d:%d PresSpeed1:%03d PresSpeed2:%03d err:%0.2f kecM1:%0.2f kecM2:%0.2f tali1:%0.2f tali2:%0.2f load1:%d load2:%d\n", 
        KP,KD,DecPoint,Lidar_data, GPIORead(8), dxl_present_speed_1, dxl_present_speed_2, c_error, kecM1, kecM2, pTali1, pTali2, dxl_present_load_1, dxl_present_load_2);
        //printf("%d\n", GPIORead(8));
