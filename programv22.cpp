...
/////////////////PID_CONTROL///////////////////////////
float Reference, c_error = 0, c_integral = 0.0, c_derivative = 0.0, c_lasterror = 0.0, c_PID = 0, KP = 1, KI = 0.0, KD = 0;
int kecMAX, kecMIN;
///////////////////////////////////////////////////////
int status_pulley1;
int status_pulley2;
...
{    
    ...
    while(1)
    {            
        json_object_set_new( root, "Drone ID", json_integer( DRONE_ID ));
        json_object_set_new( root, "lidar", json_integer( Lidar_data ));
        json_object_set_new( root, "load1", json_real( load_1 ));
        json_object_set_new( root, "load2", json_real( load_2 ));
...
void bacaencod()
{
	...
	while(1){
        ...
        pTali1 = fabs(1.11*PI * (sudut1/360));        
        encoderLast2 = n2;
        sudut2 = (float)encoderPos2/100*360;
        //pTali2 = fabs(1.105*PI * (sudut2/360));
        pTali2 = fabs(1.105*PI * (sudut2/360));
	}
}
...
void read_servolidar()
{
    ...
    {    
        ...
        if (dxl_present_speed_1 >= 1024)
        {
            dxl_present_speed_1 = dxl_present_speed_1 - 1024;
        }
        */
        //read present load//
        ...
        {
            dxl_present_load_1 = dxl_present_load_1 - 1024;
        }
        ...
        if(dxl_present_speed_2 >= 1024)
        {
            dxl_present_speed_2 = dxl_present_speed_2 - 1024;
        }
        */
        //read present load//
        ...
        {
            dxl_present_load = dxl_present_load_2 - 1024;
        }
...
void limit2()
{
    ...
    if(GPIORead(8)==0) 
    {
        ...
        PID_stop();
        if((statuslim==2)){ kecM1a=500;kecM1=500;kecM2=500; kecMAX = 700; kecMIN = 350;}
        if((statuslim==0)){ kecM1a=500;kecM1=500;kecM2=500; kecMAX = 700; kecMIN = 350;}
        while(GPIORead(8)==0){} 
    }
    usleep(10*1000);
    }
...
void PID()
{
    ...
    while(1){
        ...
        if(c_error<=0.5 && c_error>=-0.5) c_error=0;
        //c_integral = c_integral + (c_error * Dt);
        c_derivative = (c_error - c_lasterror) / Dt;
        c_lasterror = c_error;
        c_PID = (2 * c_error)+ (5 * c_derivative); //+ (0.01 * c_integral);
        ...
        //if(status_pulley1 == PULLEY_DOWN){if(kecM1<=800){kecM1 = 800;}} 
...
void baca()
{
    while(1){
        ...
        printf("status:%d:%d:%d PresSpeed1:%03d PresSpeed2:%03d err:%0.2f kecM1:%0.2f kecM2:%0.2f tali1:%0.2f tali2:%0.2f\n", 
        GPIORead(7), GPIORead(8),statuslim, dxl_present_speed_1, dxl_present_speed_2, c_error, kecM1, kecM2, pTali1, pTali2);
